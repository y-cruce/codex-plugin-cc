...
/codex:message <job-id> --queue Check the completed result against the new constraint next.
...
`message` uses native `turn/steer`: inputs stay ordered and are consumed before a later model request, not necessarily before tools already issued by the model. The acknowledgement means accepted, not executed. `--queue` lets the active turn finish and uses one pending slot to start the next turn with the message; a second queued message is rejected until that slot is delivered. `--interrupt` cancels the current turn and starts a new turn in the same job and thread with the new input and the original write permission. Existing changes remain; its report includes observed file changes and the workspace's Git status, including pre-existing edits. `--queue` and `--interrupt` are mutually exclusive.
